...
## 2026-09-08 — loop-project-fleet-integration（PR #852 第三轮 review：一个阻塞项）

- **把窗口缩短，不等于把窗口里的洞堵上** —— 上一轮我给 epoch 哨兵加了持续修复的对账扫描，
  然后在注释里写「残留只剩可用性方向」。这句话是错的，而且错得有代价：在扫描还没轮到的
  窗口里，活跃项目仍会以 epoch 0 被服务出去，对端按 0 缓存肯定授权；只要该项目在扫描到达
  前被解散（解散是先 bump epoch 再翻 status），端点又会答 0，`0 == 0` 过期检查通过，
  **授权永久保留**。修复扫描事后救不回来——它的谓词要求 `status = 1`。
  **一个「定时收敛」的机制不能用来支撑一句「结构上不可能」的断言**，两者差的正是那个窗口。
- **越是刚修完的地方，越要重读自己新写的那句结论** —— 引入这句错误注释的，正是修复上一个
  同类缺陷的那次提交。修完之后人会倾向于把「已处理」写成「已消除」。
- **同一个字段名在两个端点上不能有两种含义** —— `space_id` 在 GET 上被 trim、在 POST 上被
  拒绝。逗号分隔列表里空白是**语法**，单值参数里空白只能是**值的一部分**——理由不能跨形状
  照搬。这种分歧消费方会在生产上发现。
- **一处修好了，同类的另一处未必** —— 结尾那次 `decoder.Decode` 换成读缓冲后不会挂了，但
  **第一次** Decode 在半截 body 上照样挂，而注释读起来像是通解。用真实 socket 验证：
  没有读超时就是无限挂，加上之后 15 秒返回 400。
...
## 2026-09-08 — loop-project-fleet-integration（PR #852 第六轮：1 approve + 2 changes-requested）

- **「已记录」不等于「已交付」** —— 我把「已缓存授权没有失效通道」记进了 context.yaml 的
  open 项，以为披露就够了。两位 reviewer 同时判：不够。因为这个 PR **同时交付了一份文档**，
  把「epoch 一致 = 缓存有效」当作充分条件卖给对端。**在一份声称有界的合同旁边记一句「其实无界」，
  不是披露，是自相矛盾。**
- **异步补偿不能承载同步承诺** —— 关席位是异步的（退避几分钟 + abandoned 终态后无人认领），
  所以「等关席位时再 bump epoch」根本不是一个上限。失效信号必须与被撤销的事实**同一次提交**。
  于是给 `modules/space` 加了同步事务步骤注册点，且**步骤失败要回滚整次移除**——
  宁可移除失败让调用方重试，也不要提交一次没有信号的移除。
- **不要把「分页」的理由照搬到另一个语境** —— 级联之所以分页，是因为它共享一个 10 分钟的租约。
  事务里没有这个约束，照抄分页反而错。一条带别名的 join UPDATE 就够，N 个项目一条语句。
- **放宽守卫时，必须同时钉住「放宽到哪为止」** —— 自增守卫不认表别名，我要放宽它。
  RE2 没有反向引用，所以用捕获组在 Go 里比对两侧别名——`p.x = q.x + 1` 仍然被拒。
  放宽的同时补了三个必须被抓的别名变体。
- **守卫表达不了的形状，等于永远报绿** —— 凭据扫描的正则限定了四个前缀、四个关键词，
  于是 `OCTO_SEARCH_OS_PASSWORD`（缺 PASSWORD）和 `SPEECH_API_KEY`（前缀没想到）
  **在两个方向上都无法被分类**。放宽后立刻又抓出第三个。**假阳性的代价是一行豁免说明，
  假阴性的代价是一个没登记的凭据。**
- **对着新建的桩断言，等于没断言** —— `if store := (&stubStore{}); store.memberCalls != 0`
  用的不是路由里那个桩，这条断言不可能失败。同一个文件里其它用例都写对了。
- **桩比真库宽松，会让错误实现通过** —— 库是 `utf8mb4_general_ci`（大小写不敏感），
  桩却做精确匹配。于是「用库返回的拼写做 key」这个错误实现在桩下完全正常，而在真库上会把
  一个真实成员答成「不是成员」。**桩要模拟真库的宽松处，否则它只会同意你。**

## 2026-09-09 — internal-membership（PR #852 第八轮：第四个轴与一次被驳回的建议）

- **同一缺陷类的第四个轴，而仪器只能表达第一个** —— round 5/6/7 各找到一个 `space_member`
  的新写入方，我为此加了单表 sweep。round 8 找到的是**另一张表**：账号被全局封禁只写 `user` 行
  （撤 session、踢设备、封 IM 频道），注销也不级联删成员行，于是两行都还 `status=1`，
  `_verify` 照答 member:true 带真实 role。**sweep 扫不到它，因为 sweep 只会扫 `space_member`。**
  能改变「是不是项目成员」这个对外答案的有四张表、四种失效机制，我的 guard 表达了一种。
  现在四轴都写进 guard 头部，并写明「新轴要配自己的 guard，不是把这个 regexp 放宽」。
- **「不关席位」不是豁免理由，「双向都不改变答案」才是** —— 我的 baseline 写着「admin
  re-admission 不关席位，故豁免」，而同一个文件自述的不变量是「任何能改变成员答案的写入方
  都必须移动 epoch」。Space 合取落地之后，开席位和关席位是**对称的**答案翻转：移除窗口里
  存活的项目席位会因为重新加入而重新可达，中间没有任何项目侧写入，所以没有任何东西 bump。
  后果是对端缓存的那条**拒绝**一直和 epoch 对得上——合法回归的成员被无上界地拒绝。
  **契约缓存的是判定，不只是肯定的授权；卡死的拒绝和泄漏的授权一样是缺陷。**
- **一条 review 建议被实测驳回** —— 两位 reviewer 建议在 seat lock 里 `JOIN user`，并说
  「写路径这里是安全的，是唯一能照抄先例的地方」。**对 `lockSpaceSeatsTx` 成立，照抄到
  `lockSpaceSeatRowTx` 会静默摧毁三个创建配额**：非 OF 表是一致性读，会在 `space` 锁之前
  冻结快照，而三个配额都在它之后 COUNT（本仓 2026-09-06 实测过：六个并发 create 全成功）。
  实测对照：加 JOIN 后事务内 COUNT 看不见并发提交（0），不加看得见（1）。所以 createProject
  走独立单表查，并补了并发建项目测试钉住配额。**先例能不能照抄，判据是语句 root 在哪张表。**
- **两个陷阱叠在同一行代码上** —— 除 read view 之外，`JOIN user` 从 `octo_*` 表出发还会撞
  1267：`user` 是 dump 导入的 0900_ai_ci，`octo_project_member` 是 migration 建的 general_ci。
  **生产报错、CI 全绿**（CI 显式建库 general_ci）。所以读路径也是单表查 + Go 侧 fold。
- **guard 钉死字面量计数，正确的改动反而让它红** —— 日志 guard 断言 `m.Error` 恰好出现 2 次。
  我正确地加了第三个失败分支（各自有日志），它报红。**一个必须被编辑才能接受正确改动的 guard，
  在教人去编辑 guard**，而下一次编辑就可能是把它放宽。改成数早返回分支数。
- **只测各个半边、不测合成体，缺陷就藏在装配处** —— collation 漂移测试分别测了单表查和
  JOIN 那一半，都绿。变异时把 `ProjectMemberships` **内部**换成必然 1267 的 JOIN：源码 guard
  抓到了，**漂移测试没抓到**，因为没有任何断言执行过合成后的函数。补端到端那条之后两者同时报红。
- **造异常环境的测试必须自带 control** —— 漂移测试里先断言「那个本该失败的 JOIN 在这个
  fixture 上真的报 1267」。变异验证证明了它的价值：把 legacy 表改回 general_ci（fixture 退化）
  时，正是这条 control 报红；没有它整个测试会静默空跑并读作绿。
- **豁免按具名符号 + 附加义务，不要放宽规则** —— 新的非锁定读确实违反了既有的「事务内读
  `octo_project_member` 必须是锁定读」guard，而那条规则的**理由**在这里不成立（它跑在
  modules/space 的事务里，是该事务首条一致性读，read view 晚于 `space_member` X 锁）。
  没有把规则放宽，而是登记具名函数，并同时断言：该函数不写同一张表、且这个名字仍能匹配到
  东西（防陈旧豁免被同名新函数继承）。三个方向的变异都验证过。
- **延后修复要"两处都记"，只记内部的不算披露** —— bot 删除轴需要产品决策，我把它记在了
  内部测试文件的 baseline 里。reviewer 指出：对端读的是 rollout doc，而 §5 自称是移动 epoch 的
  轴的**完整枚举**，全文 `grep -ci bot` = **0**，运维注意里还写着「席位泄漏不再影响授权正确性」。
  **在一份把 epoch 一致当充分条件卖出去的契约旁边记录一个无上界例外，是矛盾而不是披露。**
  现在两个未修轴都写进 §5，并写明对端要做什么（对缓存判定加独立 TTL）。
- **顺手修掉一条会把人引向崩溃循环的文档** —— §4 写「多 pod 同时启动也安全，后到的匹配 0 行」，
...
## 2026-09-09 — internal-membership（PR #852 第十至十二轮：连续三轮「上轮修复开出下轮 P1」，以及类型终结）

- **同一处决策被搬了四次，每次都对，每次都少一跳** —— 第 9 轮的修复用 Go map 判定重新
  激活（第 10 轮：per-uid `FOR UPDATE` 在循环里造成两条死锁环）；第 10 轮的修复把锁提到
  循环外、结果存 map（第 11 轮：map 以**数据库拼写**为键、用**调用方拼写**查询，
  `0900_ai_ci` 下落空而 `ON DUPLICATE` 照样命中 —— 席位重开、信号没发）；第 11 轮的修复
  改成带谓词的 `UPDATE` + `RowsAffected`（第 12 轮：布尔值来自数据库了，**标识符仍然是
  调用方的**，而它要去匹配 collation 更严的 `octo_project_member`）。
  四次搬运的轨迹是：名字 → affected-rows 惯例 → 加锁读 → 带谓词写 → **标识符**。
  **连续三轮由上一轮修复开出下一轮 P1，就不该再修第五跳。**
- **结束这个序列的是类型，不是又一处修复** —— `SeatRef` 字段不可导出、没有从 `string` 的
  转换，只能由 `ResolveSeatTx`（在写入命中那一行之后、锁还握着时把规范拼写读回来）构造；
  事务步骤注册表与 outbox 入队都只收它。改完签名，**编译器把七处调用点全部点了出来** ——
  这正是三轮源码 guard 想表达而表达不了的性质：失效模式一直是「一个没人想到的调用点」，
  而计数型 census 只能在它认识的文件里失败。
- **分歧类不能靠举例定义** —— 四个人（三位 reviewer + 我）各挑一个字符论证，三次挑错：
  重音被两份评审当成分歧实例（实测两个 collation 都折，不是）；KELVIN 被一份测成 0/0
  （是 1/0，对方测法有误）；我自己第一次复现时把 `E2848A` 当成 U+212A（那是 U+210A）。
  真正的类是「有兼容性分解且基字符是拉丁字母」—— 全角形、字母状符号（KELVIN/OHM）、
  序数符都在内，长 s 和重音不在内。**实测表进了代码注释和 rollout 文档，因为下一个人
  同样会凭直觉挑错字符。**
- **不可见字符不能写成字面量** —— 回归测试第一版里，`modules/project` 那条 KELVIN 被写成了
  ASCII `K`，而 `modules/space` 那条是真的 U+212A。ASCII `K` 在两个 collation 下都折，
  于是那条用例**通过了而什么都没测**。抓到它的不是 review，是我在测试里写的 control 断言
  （「漂移拼写在严 collation 下必须匹配不到」）。两个文件后来都改成显式 `\uXXXX` 转义。
  **控制组不是仪式：它抓的第一个缺陷是测试自己。**
- **我在 commit message 里声称了一个没做的修复** —— 第 11 轮的提交写着「匹配器 header 把
  re-admission 列为无需 bump，那是完全说反了，已改」，而 diff 里**一行都没碰那个 header**。
  三位 reviewer 跨两轮读过这段声明，直到第 12 轮才有人去核字节。我随后又把这条当作已完成
  转述给了分支所有者。**commit message 不是变更。** 校正连同这段经过一起写进了那个
  header 本身，因为下一个读者需要知道的不是结论而是「这里的文字曾经骗过三个人」。
- **两位 reviewer 汇聚给出的修复方案里有一处共同的错误论证** —— 都写「由不变量 I1 保证
  规范拼写天然匹配」。I1 说的是成员关系，不是字节相等；`admitMemberTx` 写的是调用方拼写。
  真正挡住漂移拼写进入 `octo_project_member` 的是准入门那次 `FoldedHas` 折叠 fail-closed。
  那是跨两个包、无人维系的耦合，且在一个**看起来是改进**的变更（把 `FoldID` 改成宽度感知）
  下静默失效。所以准入侧也写规范拼写 —— **消除依赖，而不是把依赖记下来。**
- **两位 reviewer 各说了一半的事，合起来才是缺陷** —— 一位要求 `approveJoinApplyAtomic`
  的第 2 步加 `FOR UPDATE`（对，它现在会让下方那句关于 read view 的注释成立）；另一位
  提到这条路径是四扇重新激活门里唯一没有 `RetryOnLockConflict` 包装的（对，且标为「既有、
  本次未变」）。**只做前者会把后者从潜在缺陷变成实际缺陷** —— 加锁定读之后 1213 会直接
  冒泡给审批人。两条一起做。
- **把「今天没有」的注解换成性质** —— census 的行注释剥离器原本会截断含 `//` 的字符串
  字面量，注释里写着「被扫的 SQL 里没有」。对**计数型** guard，漏计方向就是隐藏写入方。
  改成引号感知（约十行）并补了行为 pin。块注释仍然不处理，这不对称是刻意的：树里真的存在
  行注释里的未配对 `/*`（`/v1/bot/*`），而天真处理块注释正是上一轮删掉一条真 INSERT 的原因。
...
测试覆盖缺口，明确当作「有主的 fast-follow」。本次把它们连同我自己查出的一处指标命名不一致
一起收掉。

**守卫「通过」的理由可能与它要保护的东西无关。** 普查守卫维护着一份「必须持续认得出的原语」
清单，正是为了防止匹配器烂掉之后测试变空。`deleteCreatedBotArtifacts` 一直在清单里、每次都被
找到 —— 而匹配器对它**实际使用的那种写法**（`DeleteFrom("robot")`）是瞎的：它能被认出来，
靠的是 fail-closed 兜底里一条不相干的裸 `UPDATE`。断言不空洞、结论也正确，却仍然没有能力发现
它本该发现的缺陷，因为那个样本走的是另一条路径。检验方法还是那一个：做变异，而且要用你声称
覆盖的那种形状去做，不是用你已经处理的那种。

**分类错了，修复能通过为它自己写的变异。** 「分裂到两个函数里的停用」第一版被归成「原语」——
而原语按规则不当作门，又没有人调用它，于是什么都没断言，变异全绿。正确的归类是**门**：那种
形状下删除点就是持有字面量的那个函数，它借用的 `UPDATE` 在一个通用 helper 里，那个 helper
本身不是删除。同样的代码、同样的覆盖，结论相反 —— 分类是承重的，看起来却只像命名。

**「同一条可重试错误」说的是输入，不是错误本身。** 收敛动作在预算用尽时返回级联那条错误，
注释写着两者等价。错误确实是同一个，含义不是，而差的正好是让重试有意义的那半：级联查活跃
席位，关掉的行会离开自己的结果集，每次重试输入都更小；收敛动作故意不带 status 过滤（带了就
返回空集），于是重试永远重走第一页，把整条工单的尝试次数烧光，连带已经成功的每个步骤重跑
二十遍。两个调用点可以共享错误值、重试机制和分页预算，却在「重跑有没有用」上完全不同。对任何
「稍后重试」要问的不是它返回什么，而是**下一次会有什么不一样**。

**非锁定读收窄窗口，不关闭窗口 —— 而显而易见的那个修法是一条死锁。** 事务内重读把行读进
本事务的读视图，此后提交的写对它**不可见**，而「不可见」不等于「被挡住」：随后拿的
`group_member` 锁不会回头校验 `group` 行。要的是「之后的写会等我」，不是「我看不见之后的写」。
给 `group` 行加共享锁看起来免费——它正落在声明的锁序里。

不免费，而这才是这条更值钱的一半：P1 级联先锁 `group_member`，无人可继任时再对**同一个**
`group` 行取排他锁，顺序是 group_member → group；本函数加锁后是 group → group_member。两条路
都能作用到同一个全员群（级联那条 detach 分支要的「群里没有别的项目成员」，单人项目在 owner
离开时正好满足），并发即 ABBA，MySQL 杀掉一个。于是撤掉锁，把窗口写清楚：剩下的终态是一个
Space 直属、群主可能不对的群，而 Space 直属就不再受 D7 保护，一次普通转让即可修复；真正要
先解决的是级联那处对声明锁序的倒置，那是另一条改动，得带自己的分析。

两点值得留下。**「它在声明的锁序里」说的是一个调用点，不是整个系统** —— 顺序只有在每条路径
都遵守时才成立，而没遵守的那条正是原因。以及**通过了全量套件的修复仍然可能是错的**：这次什么
都没红，是去读另一条路径的加锁顺序才看出来的。

**一个指标用了和它六个兄弟不同的命名空间。** D7 守卫的失败计数器注册在 `octo_project` 下，
按的是表前缀；同一个功能的另外六个指标在 `project` 下，而全仓的习惯是一个模块一个命名空间
（`group`、`dmwork`）。偏偏它是运维最需要告警的那一个 —— 守卫 fail-open，只有它能区分
「数据库抖了一下」和「守卫从周二起就没生效」。功能还默认关着、没人建过 dashboard，现在改名
是免费的；再晚就是破坏性重命名。

## 2026-09-09 — project-p2-all-member-group（D16 计数口径在 GA 前对齐）

核实"`member_count` 语义变更是不是 breaking"时查出另一件事，顺手在 GA 前改掉。

**「这是破坏性的吗」和「这是对的吗」是两个问题，而前者会把后者盖住。** `member_count` 被列为
本功能唯一会明显破坏客户端的改动，讨论全部围绕影响面：多少端在读、退路多贵、要不要先量。
量出来是零——但我给的理由是错的，第五轮 review 用仓库自己的 tag 拆穿了。我写的是"模块从未
正式上线"；实际上 v1.18.0 发布了 `modules/project` 的 42 个文件，包含这个字段，取自
`countActiveMembers`，也就是总数。从未发布的是"只数人"那次重定义。结论成立而论据不成立，
且真正的论据更强：这是把**已发布**的含义改回来、退掉**从未发布**的那个。
**一个建立在"状态"（从未 GA）而不是"可数事实"（没有任何 tag 包含这个提交）上的理由，
别人无法复核，只能重做一遍。** 按提出的那个问题，到此为止：不破坏，不用动。

问第二个问题才发现第一个问题看不见的东西：同一个服务里 `modules/opanalytics` 早就在发
`member_count`（总数）/ `human_member_count` / `agent_member_count`，面向同一批客户端。
所以这个改动不是破坏性的，是**不一致**的，而且是静默失败的那种不一致：两边都是 int、都能编译，
客户端作者把一个模块的约定套到另一个上就渲染出错的数字，两边各自的测试谁也抓不住——因为
单看任何一边都没错。

修法就是当初为"并不存在的破坏"设计的那条退路（恢复 `member_count`、补两个拆分字段），
但采纳理由完全不同。值得记一笔的是：如果第一个问题的答案是"是，破坏性的"，同样这个修法会落地，
而这个命名冲突会**没人发现**——因为答案会在任何人去想"这个字段本该是什么意思"之前就到了。

当初改口径的论据本身是对的，代码里也留着：一个人加他两个分身不该显示成"3 人"。但那是一句关于
**客户端该显示什么**的话，却用**改服务端该表示什么**来回答了。这两件事足够相邻，可以互相替换而
没人察觉，代价是一个名字。

## 2026-09-09 — project-p2-all-member-group（PR #868 三份 review 的修复）

两票 APPROVE、一票 CHANGES_REQUESTED，门禁最终 approved。但那条 P1 成立，而且和另一位
reviewer 的两条 P2 是同一类，一并修掉。

**守卫可以按拼写画线，再用语义解释它。** 普查匹配的是字面量 `"status": 0`。仓库里两个活的
停用入口都不带字面量——一个把解析出来的参数赋给结构体字段，一个放进调用方给的 map——而前者
最终落库的那条 UPDATE，和守卫认定为「删除原语」的那条**字节等价**。「原语」与「不可见」之间
的全部差别，只是那个 0 打在哪里。

让它变得牢固的不是错误本身，是注释：它写着这个不匹配是**正确的**，因为那些端点「是可逆的停用，
不是删除」。这句话谈的是含义，它解释的行为谈的是语法；而被拿来对照的 `deleteRobotSoft` 同样
可逆，所以这个区分连自洽都做不到。一个来查守卫的人会读到一个理由，然后停下。
**为你没有验证过的行为写下的辩护，比不写更糟**：空白会邀请下一个人去看，一个像样的理由会告诉
他不必看。

**没有样本的匹配分支，等于已经停止工作。** 第十一轮发现普查「因为错误的理由通过」；修复加了三个
分支。reviewer 把三个分支逐个删掉，套件依旧全绿——因为今天的仓库里没有任何代码是那三种写法。
于是修复带上了它所修的那个性质，只是高了一层。结果断言（「每扇门要么走 D14 要么登记豁免」）看不见
这件事，因为对一个什么都检测不到的匹配器来说，只要树里也没有可检测的东西，这句话就是真的。
两者只有在**有东西直接驱动检测路径**时才独立——而在这里那意味着从源码字符串解析出来的 fixture，
因为一个「树里还没有的形状」，树本身供不出样本。

**放宽匹配器会告诉你它一直在悄悄归档什么。** 改成按效果判定之后当场冒出一个误报：一个把自己
构造的 map 交给 SetMap 的函数，键全是 agent_*，永远没有 status。第一版规则写的是「非字面量」，
那不等于「调用方的」：本地构造的 map 列是可知的，形参不是。收紧成「实参是本函数的形参」之后，
顺带**移除了旧规则本来就在制造的一类指控**——`setDescription`、`updateBotCommands` 这些的调用方
本来也会被报成删除 bot 的门，并被要求去调 `CloseAllSpaceSeats`，而那对它们是错的建议。
误报不是放宽的代价，它本来就在，只是藏在一个更窄的触发条件后面。

## 2026-09-09 — project-p2-all-member-group（PR #868 第三轮：豁免必须被断言消费）

两位 reviewer 各自做了同一个变异：删掉「状态可写函数的调用方算作门」那个循环，套件全绿，门集从
6 降到 4，两个活的停用端点无声地退出了普查。上一轮刚加的整套机制，是可删除且不变红的。

**一条登记的豁免，如果没有任何断言去消费它，和一个被删掉的匹配器无法区分。**
`d14ExemptDoors` 里那两条读起来像断言——它们写着门的名字、写着残留、写着为什么不修。但它们是
map 的键，只在「这扇门被找到了」时才会被查到；门没被找到，键就只是配置。而反向的锚点（已知门
清单）只列了那三扇通过原语可达的，正好绕开了这两扇。修法是三行：遍历 `d14ExemptDoors`，
要求每个键都出现在实际找到的门里。**豁免的名字本身就是最好的锚点，因为它记录的是一个决定，
而决定必须持续为真。**

**上一轮补的是分类器，不是装配。** 「没有样本的分支等于已停止工作」这句话，上一轮只兑现在
`censusFuncsIn`（一个函数是什么）上；「哪些函数成为门」那一半仍然只由仓库碰巧包含的东西驱动。
两条装配规则都因此可删：删掉「没人调用的原语是它自己的门」，门集逐字节不变，因为今天三个原语
都有调用方。同一个缺陷在三个抽象层重复了三次——普查（第 11 轮）、分类器分支（第 1 轮）、装配
（第 2 轮）。把 `assembleCensus` 从测试体里拆出来、用合成包驱动，是为了不出现第四次。

**标语会替代测量。** 头注释写着「删掉任意一个检测分支，下面正好有一个 case 变红」。真去把九个
分支逐个删一遍：八个成立，第九个不成立——`Update("robot")` 那个表判断会红五个，因为它不是一种
拼写，是所有 robot 表形状共同的前提。写下来时它是一句漂亮的话，量过之后才知道哪一半是真的。
现在头注释写的是测量结果，以及那个真正重要、也确实被验证过的性质：**没有任何一个分支是可以
删掉而保持全绿的。**

## 2026-09-09 — project-p2-all-member-group（PR #868 第五轮：一次测了个寂寞的变异扫描）

重构完 `matchRawSQL` 之后重跑分支变异扫描，十一个分支全部返回 **0 红**。那读起来是「每个分支都
可以删掉而保持全绿」——最坏的结果——而它是假的：上一个包跑完在共享 test 库里留了迁移状态，
`TestMain` 在任何用例体执行之前就 panic 了，而一个靠数 `--- FAIL` 行的脚本，**分不出「没有失败」
和「没有运行」**。

**一个只统计结果行的变异脚本，本身就是一个没有自检的守卫。** 这个文件花了四轮在讲「断言必须有
一条会因它为假而变红的检查」，而我用来验证这件事的工具，恰恰缺这条检查。现在扫描先建基线、记下
子测试数量，任何一次变异只要子测试数量对不上、或输出里有 panic / 编译错误，就记为 **INVALID 而
不是绿**。

侥幸的地方值得说：因为十一个全是 0，太整齐，我才起疑。**要是它返回的是一组看起来合理的数字，
我就信了，并且会把它当事实写进头注释。** 一次坏掉的测量和一次真实的测量，长得一模一样——除非
你让它必须先证明自己跑过。

**顺带一条**：这轮加的字符串拼接折叠，第一版遇到非字面量片段就整体放弃，理由是「不要跨空隙
臆造相邻关系」。变异测试指出这个取舍是反的——放弃会**漏掉真实的停用语句**，而跳过的坏处只是
...
## 2026-09-08 — loop-project-fleet-integration（出站半边：O1a/O4/O5/O6/O7 + 契约冻结）

- **一张表的唯一索引就是它的形状，不是它的实现细节** —— 分支笔记里写着「生命周期事件搭载
  #850 的发件箱」。真去看时才发现 `octo_project_provisioning` 是
  `UNIQUE (project_id, target)`——**一个项目每个目标只有一行的映射表**，装不下 append-only
  的事件流。这不是「改一下就能用」，是两种互斥的形状。**照搬一条计划之前，先读它依赖的那个
  约束。**
- **测试改到实例上，请求却发给另一个实例** —— 生产者用例把开关写进 `setup()` 返回的 `*Project`，
  但 HTTP 走的是共享 `testSrv`，上面挂的是 `module.Setup` 注册的**另一个实例**。四个用例
  三个红，症状看起来像「入队没接上」。**改实例字段的用例必须挂私有路由（`mountProject`）。**
- **注释能满足的守卫，等于没有守卫** —— 我给 `ProjectEpochsInSpace` 加了
  `activated_at IS NOT NULL`，配了源码守卫。做变异测试时把 SQL 里那段删掉——**守卫照样绿**，
  因为函数体内解释这个过滤器的注释里也有这个字符串。守卫现在先剥掉 `//` 注释再搜。
- **外层枚举挡住的分支，不算被测到** —— 级联重跑用例断言「不会重复入队」，看起来在钉
  `if changed` 分支。把入队移出该分支后**用例仍然绿**：外层 step 重新查「活跃席位」时一无所获，
  根本没走到这个函数。改成直接调 `deactivateSeatForCascade` 才真正把分支放到镜下。
- **dbr 把 NULL 解成非 nil 的零值指针** —— `Load(&[]*time.Time)` 读一个 NULL 列，拿到的是
  **指向零时间的非 nil 指针**。于是「未激活」和「激活于公元零年」不可区分，第一版测试助手
  把每个未激活项目都报成已激活。模型字段也改成 `sql.NullTime`：**NULL 必须落在值域之外**，
  这正是 `member_epoch = 0` 哨兵当初必须离开值域的同一条理由。
- **契约画的是状态机，不是列布局** —— 契约 §7 画的是 `status=provisioning`。真按第三个 status
  值实现，会一次性改变约二十处 `status = 1` 谓词的含义，而它们表达的是五件不同的事。
  改用 `activated_at` 列：**对端观察不到任何差别**，漏改一处的代价从「成员静默变非成员」
  降到「对端早看到一小会儿」。§8 给归档选列是同一条论证，这只是把它用在同一张表上。
- **两个「等价」的校验，先确认它们是不是同一个值** —— 契约要求校验
  `workspace_id == project_id`。代码里已有的是「目标回显的 id == 我方要求创建的 id」。
  这是**同一个校验**，但两个 id 目前**不是同一个值**：fleet 容器 id 仍是不可推导的随机形式，
  而那个不可推导性是 P-3 提权链路上唯一的阻断。所以顺序不能反——**对端先收窄，我方后换 id**，
  否则「按契约实现」就等于亲手打开那条链路。
- **默认关闭的功能，它的闸门默认必须是开的** —— 两阶段创建等的是 fleet ensure。fleet target
  默认不开，于是没有任何东西会去置位闸门。如果照着「新项目默认未激活」写，
  **今天所有部署的每一个新项目都会对对端永久不可见**。闸门跟着 target 走，不给它自己的开关——
  开关能在 target 关着时被打开，那正是会搁浅项目的状态。

## 2026-09-08 — loop-project-fleet-integration（code review 修复）

- **两个地方各自判断「配置可用吗」，就会有一个判错** —— 入队闸门只查 URL/secret 非空，
  投递客户端却校验 URL 形状和密钥长度。于是「URL 带 query」或「密钥 20 字节」这类配置
  **闸门放行、客户端永远建不出 sender**：claim 在投递前就 `attempts+1`，所以每个 tick 烧掉预算
  却永远走不到 abandon 分支，表无界增长（purge 只清终态），撤权一条都发不出去。
  现在两边调同一个 `validateLifecycleEndpoint`，并加了一条测试直接钉「客户端拒绝的，闸门必须也拒绝」。
- **全局有序 ≠ 每个项目有序** —— `project.created` 撞上瞬时 503 被退避后掉到队尾，
  同项目后面的事件先发，对端答「未知项目」——一个普通 4xx，本客户端判**终态**。
  于是一次瞬时故障就能**永久放弃**它后面的 `member_revoked`。改成「按项目阻塞，首次失败即停」：
  阻塞是每项目的，别的项目不受影响；被跳过的行退还 claim 记的那次 attempt，
  否则一个头部事件慢的项目会在没投递过一次的情况下把自己的尾巴耗光预算。
  `abandoned` 算「完成」——它永远不会重试，卡在它后面等于把一次丢失变成项目永久停摆。
- **按字节截断会切出半个 UTF-8 字符** —— `last_error` 里有对端返回的错误码，未必是 ASCII。
  切坏的字符串在 strict 模式下报 1366，而这条写入正是「标记已投递/已放弃」的那一条：
  行标不上终态 → 租约过期 → 同一事件无限重投。**乱码是化妆品问题，标不上终态不是。**
- **反应式路径写在终态之后，就必须有补救** —— `confirmProjectActive` 在 job 已置 `ready`
  之后才置激活闩锁，而终态 job 不会被再认领。中间被杀或那条 UPDATE 报错，
  项目就**永久**对对端读作不存在，只有 gauge 没有修复。换个顺序只会换成更坏的失败
  （job 未完成却已对外可见），所以答案是加一条对账扫描，不是调顺序。
- **MySQL 不允许多表 UPDATE 带 LIMIT** —— 修复语句原本是一条 join UPDATE + LIMIT，
  **编译通过、运行时报 1221**，日志里一条 Warn，修复从来没跑过。是测试抓到的。
  改成 SELECT 取 id（带 LIMIT）再 UPDATE。为了让语法过关而去掉 LIMIT 不是选项：
  这里的行数就是积压，**唯一它大的时候，正是它唯一重要的时候**。
- **「命名了字段」不等于「改了东西」** —— 改资料只看字段有没有出现，于是重发同一个名字
  也会 bump `lifecycle_version` 并入队一条事件。队列是每 5 秒 10 条串行网络往返，
  这些空事件会排在 `member_revoked` 前面——一个项目管理员就能驱动的放大器。
  拆成 `named`（决定 400）和 `set`（决定做不做事）：命名了但值都相同，返回当前行，不写不 bump 不入队。
- **写新代码时，先看它会不会撞上别人立的守卫** —— 激活修复要读 `octo_project_provisioning`，
  被 #850 的两条守卫拦下（该表只允许出现在 `db_provisioning.go`；容器 id 不得被切片外的代码读到）。
  守卫是对的：把函数搬进 `db_provisioning.go` 就行，而且查询只 SELECT `project_id`。
  **绕过守卫之前，先确认自己不是它要防的那种情况。**

## 2026-09-08 — 重建到 #852 新 head

- **别人的分支被 rebase 了，你的基点就不存在了** —— 我这条分支建在 #852 的 `f6d60d7e` 上。
  对端把 #852 整条 rebase 到新 main，SHA 全变，于是两条分支的合并基点退回到一个 main 老提交，
  `git diff` 无论以谁为 base 都会把**我这份 #852 旧副本**整个算进去：63 files / +11295。
  重放到新 head 之后是 40 files / +4743，**才是这一片**。
  推送前看着没问题的 diff，不代表开 PR 时还是那个 diff。
- **两条独立的理由，会撞出同一个修复** —— 两边都给 `pkg/project` 的源码守卫加了「先剥注释」。
  对端是因为**缺席断言**（`ProjectMemberships 不得出现 space_member`）被函数内一句合理的注释永久破坏；
  我是因为**存在断言**（`activated_at IS NOT NULL`）被解释它的注释满足。
  同一个属性，两个反方向的失败。合并时把两条理由都留下——只留一条，另一半会在下次被人「顺手简化」掉。
- **报出去的问题，要回头确认它是不是已经被别人修了** —— 我上轮报的 #4（map key 大小写）
  和 #6（锁顺序死锁）对端已在 `ea51fd1c` 修完，而且诊断比我的更准：那条 join UPDATE 的锁顺序
  **根本不是 SQL 的属性**——驱动表由优化器按基数选，实测 3 个项目时 `p` 驱动、200 个时 `pm` 驱动，
  顺序会翻转。我原来的描述（「它固定先锁 member」）说窄了。

## 2026-09-08 — PR #864 第一轮 review（CHANGES_REQUESTED，4×P1）

- **「同一个对端、同一套签名方案」的第三个客户端，不该靠记性写对** —— 出站客户端没清 Proxy、
  没拒重定向，而同仓另外两个签名客户端都拒了，其中一个还是发给**同一个对端**的。
  最坏的不是失败而是**假成功**：Go 把 302 的 POST 改写成 GET 并丢掉 body，目标返 2xx →
  被判 `OK` → 行标记「已投递」、计数器 +1、积压下降，**而对端从没收到**。
  撤权永久丢失还报成功，比放弃那条路更糟。校验器同理，是兄弟校验器的真子集，
  少了 scheme 白名单——`htps://` 这种一个字母的手误能过闸门，然后每条事件烧 12 次退避后放弃。
- **内存里的顺序保证活不过一次 tick** —— 我上一轮的「按项目阻塞」是延迟循环里的一个 map。
  它在一个批次内成立，**下一个 tick 就反了**：失败的头被退避到 `now+backoff`，
  被跳过的尾却带着原始 `next_attempt_at` 交还，而 claim 按 `next_attempt_at` 排序——
  尾巴排到了头前面，且第一个退避窗口内头还没到期，尾巴被**单独**认领，map 是空的。
  评审给的复现只是「再调一次投递循环」，我跑了，确实。
  **顺序约束必须放进 claim 查询**：那里它还能跨副本成立，而进程内的 map 根本看不见别的 pod 持有的租约。
- **加了一个限制，就要检查它有没有制造新的死角** —— 给 claim 加 `attempts < max` 之后，
  一个「预算耗尽但从未终态」的行变成**不可认领**；而 claim 又拒绝任何有更早 pending 兄弟的项目，
  于是这一行会**永久堵死它所属项目的整条队列**。所以谓词和清扫必须成对出现，不是可选的补充。
- **一次性 backfill + 可空列 = 滚动升级会漏** —— 首个 pod 跑完迁移到最后一个旧 pod 退出之间，
  旧二进制按旧列表插入，行落在 backfill **之后**，`activated_at` 是 NULL。
  而修复扫描要求存在 ready 的 fleet 预置行——fleet target 关着时永远不存在。
  这些项目对对端**永久**不可见。补的谓词就是创建时已经在用的那一条：
  没有任何东西会去确认它们时，直接闩上——替代方案不是「以后确认」，是「永远看不见」。
- **契约与代码的分歧，在哪一边都是缺陷** —— 契约说 `project_version: null`，代码是 `omitempty`
  **整个键不出现**。Go 两边都解成 nil 指针，所以看着不要紧；但对端照 §2 字面写了 schema 校验就返普通 4xx，
  而普通 4xx 在我这边是**终态**——首个被放弃的正好是 `member_revoked`。
  `occurred_at` 同理：列是 `DATETIME(3)`，`time.RFC3339` 把毫秒丢在了线上。
- **改了服务层，别忘了调用方那一半** —— 空改不再写库了，但 handler 仍在 `err == nil` 时无条件写审计，
  于是「审计记录了一次没发生的变更」——正是 400 分支自己的注释反对的那件事，从另一扇门回来了。
