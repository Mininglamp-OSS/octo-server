...
description: A unique key guarantees one durable projection identity but concurrent multi-transaction reconciliation can still deadlock; retry the complete idempotent orchestration on MySQL 1213 only.
...
check after MySQL 1213. Do not retry arbitrary errors, do not retry only
...
## External subscriber projections

An external channel "create or update" API may be additive rather than
replacement-based. Never infer exact-roster semantics from a request containing
the full desired subscriber list. Persist the authoritative roster, serialize
projection per owner, re-read after acquiring that serialization fence, add the
current desired set, and explicitly remove every known excluded identity from
the parent and child channels. Persist a retryable failed state for partial
external failures.
