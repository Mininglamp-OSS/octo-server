...
- A reconciliation-triggering GET must not become an availability dependency on
  the external IM service. Ready rows skip replay; failed repairs stay visible
  through persisted state and can be retried without blanking the list.
- External projection must not hold a transaction, pooled connection, or MySQL
  named lock across WuKongIM I/O because the legacy IM client has no deadline.
  Each roster mutation advances a durable `roster_version`; a short
  post-projection transaction marks readiness only when that version is still
  current, and otherwise replays the latest snapshot. Failed/provisioning reads
  use a cooldown so an IM outage is not amplified by every page load.
...
- Authorization compares Bot identities as a set rather than trusting database
  ...
  same contract.
- A projection version only protects inputs that advance it. Lifecycle cleanup
  therefore deactivates affected Agents and advances the aggregate roster
  version in the same transaction as protected membership removal, while the
  ready CAS also rechecks effective eligibility before acknowledging external
  convergence.
