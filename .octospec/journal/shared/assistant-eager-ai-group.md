---
type: Journal
title: "Journal: assistant-eager-ai-group"
description: Eagerly provisions each AI Team Agent's private owner-and-Bot group and wires assistant creation to the authenticated Agent endpoint.
tags: [ai-team, space, auth, group, idempotency, assistant]
...
# Journal: assistant-eager-ai-group
...
The companion client change calls the Agent endpoint after User Bot creation with
...
- IM failure happens after the group transaction commits. The durable Agent and
  group remain retryable, but the client must not mark the local binding successful
