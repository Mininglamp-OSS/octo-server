...
- **P0** — 统一 archived 子区在**会话列表**读路径上的可见性谓词：
  `/v1/conversation/sync` 与 `/v1/sidebar/sync?tab=recent`。**follow tab 不在其中**
  —— 它是客户端唯一的关注状态真源，收口会破坏取消关注，详见下方 Acceptance 里那条
  被撤销的验收项。原文写作「三条读路径」，是本 brief 起草时的目标；评审推翻后已只覆盖两条。
