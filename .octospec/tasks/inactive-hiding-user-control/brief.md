...
1. **隐藏窗口默认值**（产品决策）。`daysCutoff` 用自然日（`now.Add(-days*24h)`，
   `api_sidebar.go:773`）。周末会吃掉窗口的大部分：**周一早上 9 点回看，3 天窗口只
   记得到周五早上 9 点**，所以周四下午还在讨论的话题，周一上班就已经不在列表里了 ——
   跨一个周末，3 天窗口实际只剩约一个工作日的记忆。该值在 Batch 2 后将成为**所有
   用户的初始默认**，届时更改成本更高。建议 **7 天**：周一回看恰好覆盖上一个完整
   工作周，且无需引入工作日/节假日日历。
2. **归档窗口与隐藏窗口的目标关系**。加上 P2 未读豁免后，两层构成两级衰减：`N 天且无未读 → 从我的列表淡出`，`M 天 → 全局归档`，需 `M > N`。当前 M = N = 3，建议拉开（Discord 最长档 7 天可作锚点）。具体取值待定。
3. **移动端是否使用 `/v1/sidebar/sync?tab=recent`**。本仓库只有 web + extension；若独立移动端拿它当主列表，P0 对移动端即为可感知变更。走 `conversation/sync`（不传 `recent_filter`）则不受影响。**不阻塞 Batch 1 实现，合并前确认。**
