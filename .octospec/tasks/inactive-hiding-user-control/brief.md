...
1. **隐藏窗口默认值**（产品决策）。`daysCutoff` 用自然日（`now.Add(-days*24h)`，`api_sidebar.go:773`）——周五下午最后一条消息，周一上午即出窗，跨周末必然误伤。该值在 Batch 2 后将成为**所有用户的初始默认**，届时更改成本更高。建议 7 天（覆盖完整工作周）或改按工作日计。
