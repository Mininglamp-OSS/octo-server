...
- [ ] `git diff --stat` for the PR touches no file under `modules/group/`,
      ...
      **Amended (round 4):** plus two more, both of which were already in the diff and
      should have been recorded here rather than found by a reviewer:
      `pkg/authtree/authtree.go` (+19 lines, **comment only** — records why
      modules/project's routes are contributed to NO authtree: User API Keys stay
      Space-scoped and a bot has no Space seat, so neither gains a Project dimension.
      Written there rather than left as the absence of an `Add` call) and
      `tools/i18nmarkers/server/active.en-US.toml` (generated by `make i18n-extract`,
      therefore mandatory whenever a code is added or removed — not a discretionary edit).
...
> **Where the per-item verdict lives (added round 4).** These boxes are deliberately left
> unticked: the authoritative, per-criterion verdict — with the specific test or reasoning
> discharging each one — is the table in
> `.octospec/tasks/project-p0-foundation/verification.md`, and duplicating it here would
> mean maintaining the same state twice. That is not hypothetical: round 4 found
> `verification.md` citing three test functions that had never existed, which is exactly
> what dual bookkeeping produces.
>
> Current tally from that table: **29 met**, 5 deviating-with-reason, and the invitation /
> self-join criteria moved to the next phase along with the surface they describe. The two
> items that are NOT dischargeable by code are called out separately — the collation
> conversion above, and the ownerless-project product decision in Open questions.
...
## Collation: measured, and the answer is a pre-enable blocker

Checked against the production database on 2026-09-06. **All four joining columns are
`utf8mb4_0900_ai_ci`, not the `utf8mb4_general_ci` this module pins:**

| table | column | production collation |
|---|---|---|
| `space` | `space_id` | `utf8mb4_0900_ai_ci` |
| `space_member` | `space_id`, `uid` | `utf8mb4_0900_ai_ci` |
| `user` | `uid` | `utf8mb4_0900_ai_ci` |

**Cause: the historical data import.** `mysqldump` omits `COLLATE` for a table whose
collation equalled the source database default, so those tables inherited MySQL 8's
default on restore, while every migration-created table kept the explicit
`utf8mb4_general_ci` it declares. So `general_ci` is the intent and `0900_ai_ci` is an
import artefact — not two competing conventions, which is what makes converting the
right direction rather than a coin toss.

**Consequence if this module is enabled first:** `listMembers` (`LEFT JOIN user`) fails
with error 1267 on every request, so `GET /v1/projects/:project_id/members` is a 500 for
every project unconditionally, and all four reconcile scans go dark together. CI cannot
detect it: `ci/run-e2e-shard.sh` creates its database with an explicit
`COLLATE utf8mb4_general_ci`, so a green suite is structurally not evidence.

**The fix is not per-table.** `user` / `space` / `space_member` also join `robot`,
`group_member`, `group`, `app_bot` and the opanalytics dimension tables. Converting only
these three would break whichever of those is still `0900_ai_ci` — they are compatible
today precisely because the dump left both sides alike. The whole dump-imported set has
to move together, and `TABLE_COLLATION` other than `utf8mb4_general_ci` identifies it
exactly.

A second, unrelated symptom of the same drift, found while investigating: on a
default-collation database the migrations **cannot be replayed from zero** —
`20260416000001_category_legacy01.sql` fails with 1267 on
`UPDATE group_setting gs INNER JOIN group_category gc`. Existing instances survive only
because those tables predate the divergence. That will bite the next new environment and
deserves its own issue; it is not this PR's to fix, but the conversion below also fixes it.

### Pre-check (read-only, must all return 0 rows)

```sql
SELECT CONCAT('SELECT ', GROUP_CONCAT(CONCAT('`',s.COLUMN_NAME,'`') ORDER BY s.SEQ_IN_INDEX),
              ', COUNT(*) c FROM `', s.TABLE_NAME, '` GROUP BY ',
              GROUP_CONCAT(CONCAT('`',s.COLUMN_NAME,'` COLLATE utf8mb4_general_ci')
                           ORDER BY s.SEQ_IN_INDEX),
              ' HAVING c > 1;') AS precheck
...
WHERE s.TABLE_SCHEMA = DATABASE()
  ...
  AND s.NON_UNIQUE = 0 AND c.COLLATION_NAME IS NOT NULL
GROUP BY s.TABLE_NAME, s.INDEX_NAME;
```

`general_ci` has coarser equivalence classes than `0900_ai_ci`, so two rows that differ
today can collide after conversion and hit a unique index. The columns at issue here are
system-generated ids (`uid`, `space_id`, `short_no`), which are ASCII and therefore
compare identically under both — so a collision is unlikely, but this is irreversible and
cheap to check.

### Conversion (one window, smallest first)

```sql
SELECT CONCAT('ALTER TABLE `', TABLE_NAME,
              '` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;') AS stmt
...
Changing a collation rebuilds the table, so size decides the window. **Converting in
batches has a live-broken middle state** — already-converted tables stop matching
...
Turning the kill switch off stops new Projects and freezes all membership writes
while leaving existing rows readable. Full revert is dropping the two
`octo_project*` tables and removing the blank import from `internal/modules.go`:
nothing else in the codebase references them, no existing table gains a column,
and no existing row is modified — which is the whole reason P0 is scoped this way.

**Amended (round 4) — the switch is not a live toggle.** It shipped as the
environment variable `OCTO_PROJECT_CREATE_ENABLED` (plan §5.1 option A), not as a
`system_setting` row (option B, which the plan itself recommended). So flipping it
**in either direction requires a rolling restart**. The text above previously read
as if it were pullable at runtime; it is not. Two consequences to plan around:

- The mitigation path for anything found in production is a deploy, not a config
  change. Budget for that before enabling it.
- The default is `false`, which is the right default and is what bounds the blast
  radius of every open item below — including the collation one.

Moving it to `system_setting` before the feature is enabled in production would
restore the rollback story this section describes; that is a follow-up, not P0.

Also corrected here: this section said "three `octo_project*` tables". P0 creates
**two** — `octo_project_invitation` moved to the next phase with the rest of the
invite surface.
