...
自定义 AI 群保留普通群的受限管理与消息能力，但自动全员群和所有自定义 AI 群均只在
“我的 AI 团队”专属入口展示；不得泄露到普通最近会话、通讯录、群列表或分类树。任何
入群路径都只能添加当前用户在当前 Space 内有权使用的 AI，不能加入其他人类。
...
- 自定义 AI 团队允许群主改名、改头像、使用普通群设置、置顶/免打扰、清空本人
  历史、创建和管理子区；但其群、会话和子区只由 AI Team 专属 API/页面呈现，不能进入
  普通最近会话、普通群/通讯录、保存群聊或分类树。普通入口统一排除
  `ai_session_container`、自动 `ai_team_group` 和 `ai_custom_team_group`。自定义团队解散直接采用普通群
...
- 新 Agent 创建后自动进入全员群并出现在可选目录，但不自动进入任何自定义团队；显式添加
  ...
  则从全员群及所有自定义团队移除。
- 自定义 AI 团队能通过 AI Team 专属详情、会话和子区入口正常使用，支持普通群的
  名称、头像和个人设置；普通最近、群列表/通讯录、保存群聊及分类树均不返回任何 AI 团队群。
  普通群入口无法邀请人类或未授权 AI，也无法让唯一人类退出、
  转让群主或把团队转换为普通多人群。
