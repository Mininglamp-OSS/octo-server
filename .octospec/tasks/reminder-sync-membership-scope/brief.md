...
- **频道类型矩阵** — 频道级提醒的 `ChannelType` 从消息原样拷贝
  ...
  | 值 | 类型 | 成员来源 | 本次是否加谓词 |
  |---|---|---|---|
  | 1 | Person | channel_id 即收件人 uid，当事人可判定 | **是** |
  ...
  元数据、非权威成员关系），但**成员表不是唯一的授权来源**：类型 1 的当事人关系
  直接编码在 channel_id 里。故 1/2/5 各按自己的关系加谓词，只有 3/4/6 因全仓无
  生产者而原样放行 —— 对它们硬套 `group_member` 才会静默丢弃合法提醒。
...
- DM 跨 Space 过滤的同类 fail-open。同一类缺陷，但影响面与判据都不同（只能读到
  自己参与的 DM），单独立项。**不在此列出具体位置**：本仓公开，把未修复项的调用点
  精确到 file:line 提交出去等于公布可利用面（PR#717 review）。
- 复测报告 §4.10 `real_name` 对他人下发。经查为 YUJ-413 明确产品决策，需产品口径
  而非代码修复，同样不在此标注行号。
- 复测报告中本任务范围之外的其余条目。不在此逐条列举：本仓公开，把「确认存在且
  尚未修复」的清单连同 file:line 一起提交等于对外公布可利用面，而这些条目在复测
  报告原件里已有完整记录，无需在此重复（PR#717 review P2-6）。
- **频道类型 3 / 4 / 6 的频道级提醒不加谓词**（知情保留的残留）：全仓无生产者
  （`grep` 三个常量在非测试代码中零命中），没有可暴露的东西；而硬套 `group_member`
  会在它们将来变成真实数据时静默丢弃合法提醒 —— 功能回归而非安全收益。残留以守卫
  测试监控，见下方 Acceptance。经维护者确认后保留。

  **类型 1 Person 不在残留内**（原先在，是错的）：它的当事人可判定（收件人 uid 就是
  `channel_id`），且 DM 发送路径是可达的生产者，两位 reviewer 独立指出。已加当事人
  谓词关闭。原先的理由「硬套 group_member 会误伤」对类型 1 根本不适用 —— 它不需要
  任何成员表。
...
- [ ] **频道类型 3 / 4 / 6 的频道级提醒行为与改动前逐字节一致**（证明未误伤无生产者
      的类型）。
- [ ] **类型 1 Person 按当事人判定**：发给调用方本人的 DM 频道级提醒可见；第三方之间
      的 DM 边不可见；自己发布的仍不回给自己（与 YUJ-1377 叠加不冲突）。
- [ ] **守卫测试**锁定「能产生频道级提醒的频道类型集合」与「谓词覆盖的类型集合」
      之差（即残留集合）。收紧谓词或放宽产生方，都必须让该测试转红。
- [ ] **新增频道类型默认拒绝**：谓词是白名单而非黑名单，`common.ChannelType` 追加
      枚举值时，其频道级提醒不可见而不是全局可读。

      注：初稿在此写的是「当前写入路径只会生成类型 2/5」，**那是错的**——`hasMention`
      不判频道类型，六种类型结构上全都能产生频道级提醒。初稿还声称守卫测试能捕获
      「新增频道类型」，**同样是错的**：`allTypes` 是手写字面量，看不见新值。两处都
      由 PR#717 review P1 指出。现在的处理是分工明确：前两类改动由测试兜底，新增
      类型由**代码**兜底（白名单默认拒绝），各自有独立的判据，不再由一句话含糊承包。
