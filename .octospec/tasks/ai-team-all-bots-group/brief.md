...
description: 以 AI Team Agent 为权威名册，同步维护每个 Agent 的二人群和本人加全部 Agent 的“我的 OPT”群。
...
`(space_id, user_uid)` 自动维护一个可见的“我的 OPT”群。`ai_team_agent` 是两类群的
...
- 每个 `(space_id,user_uid)` 最多一个有效 AI 总群；并发首次访问、并发建 Bot 和重试
  必须收敛到同一个 `group_no`，不能产生两个“我的 OPT”群。首次创建通过锁定现有
  `space_member` 行串行化，不增加群映射表。
...
- Bot 创建的所有生产入口（BotFather 对话创建、`/v1/user/bots`、`MintBotOBO`）都必须在
  ...
  收敛两类群状态。
- 建群和成员写入必须复用 `group` / `group_member` 的统一写入、版本、通知和 WuKongIM
  订阅语义；DB 已提交而 IM 调用失败时必须返回可重试错误，不能把未完成的外部投影返回成
  可聊天成功，也不能回滚已成功的 Bot 创建。
...
- 一个拥有 N 个符合条件 User Bot 的用户首次进入 AI Team 时，系统为它们补齐 N 条有效
  Agent 记录和 N 个各含“本人 + 对应 Bot”的二人群，并且最终只有一个名为“我的 OPT”
