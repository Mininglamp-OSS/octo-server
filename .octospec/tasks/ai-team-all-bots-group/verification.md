...
- A ready `GET /v1/ai-team/agents` no longer replays WuKongIM writes. Failed
  lazy repairs are logged and exposed through persisted readiness state without
  turning the list into a 503.
...
- Aggregate-group projection holds no transaction, pooled connection, or MySQL
  named lock across WuKongIM HTTP. Each authoritative roster change increments
  `roster_version`; the ready/failure writes use version CAS, and a stale HTTP
  completion replays the latest snapshot. Failed/provisioning list repairs use
...
- `go test ./modules/ai_team -run 'TestAITeamProjectionReleasesDBBeforeIMAndReplaysStaleRoster|TestAITeamFailedTeamProjectionUsesReadCooldown' -count=1`
- `go test ./modules/ai_team -count=1`
- `go build ./...`
- `go vet ./...`
- `git diff --check`
...
- Bot authorization compares identities as a set, independent of MySQL
  ...
  `bot2_bot` together.
- The ready CAS re-reads effective eligibility, while protected lifecycle
  cleanup atomically deactivates affected Agents and advances the aggregate
