...
- **trust boundary (new egress)**: the provisioning call is octo-server's first
  egress to fleet or drive. One secret per target, HMAC-signed per
  `internal/cardactiondispatch/signature.go`, registered in
  `ValidateNotifyTokenExclusions`
  (`internal/cardactiondispatch/registry.go:240`, called from `main.go:682`) so it
...
4. **Signing** reuses `internal/cardactiondispatch/signature.go`'s `Sign()` — v1
   HMAC-SHA256 over `method \n path \n timestamp \n eventID \n sha256(body)` — one
   secret per target, mutually exclusive from every other internal token.
