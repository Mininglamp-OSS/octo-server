...
| E8 | **Pre-decode caps** (net-new): `http.MaxBytesReader` 1 MiB on `bot_api`/`robot` send+edit routes (protocol Decision 3b; POC only capped the P2 action route, which is not in PR-A). |
