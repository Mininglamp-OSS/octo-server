...
- Every form of subsystem integration: no `projects[]` in
  `verify?include=context`, no epoch endpoint, no removal log, no replica report,
  no callback registry, no fleet/drive work. The design brief's R3/R4/R7 stay
  unbuilt until a subsystem meets the Tier-1 entry conditions.
...
**PR-3 — the observation scans** (stale subscriber, `removing` stall) if PR-2 grew
