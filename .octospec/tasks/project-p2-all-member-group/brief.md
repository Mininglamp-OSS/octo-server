...
"这个所有者名下有几个分身"。
所以趁"只数人"这个含义还没进任何发布就改回来，并对齐 opanalytics 的命名。这是唯一免费的
窗口，而理由是能用 tag 查的，不是一句关于"有没有 GA"的断言：v1.18.0（2026-09-07）**确实**
发布了 `modules/project`（42 个文件），其中 `member_count` 取自 `countActiveMembers`，也就是
**总数**；而"只数人"那次重定义（566e625）`git tag --contains` 查下来不在任何 tag 里。所以这次
是把**已发布**的含义改回来、退掉**从未发布**的那个——v1.18.0 的客户端什么都不用改，只有对着
未发布 main 构建的客户端要改一个字段名。
（本段原本写的是"模块从未正式上线"。那句话是错的，PR #868 第五轮 review 用仓库自己的 tag
拆穿了；第六轮又发现同一次清扫改了 model.go 和 journal 却漏了这里——而这份 brief 正是
product-surfaces 指定的 Swagger 契约来源。）
...
1. ~~**`member_count` 语义变更的客户端影响面（D16）。**~~ **已了结，走了退路方案。**
   核实结果（按 tag 查，不按"有没有上线"断言）：v1.18.0 已经发布了 `modules/project` 并且
   在发 `member_count`——含义是**总数**；"只数人"那次重定义不在任何 tag 里。所以已发布的客户端
   读到的一直是总数，这次是把它改回来
...
与上面那几条不同：这两条**不是"问一下"，是"没做完就不能开"**。

1. **`listMembers` 依赖 collation 归一，D16 的花名册字段在归一前发不出去。**
   `listMembers`（`modules/project/db.go`）里 `LEFT JOIN user u ON u.uid = pm.uid` 这条
   既有比较在生产的排序规则形态下直接报 1267——也就是说这个函数在归一之前**根本执行不了**，
   而 D16 的 `robot` / `owner_uid` 就挂在它上面（新加的 `LEFT JOIN robot` 已经按归一后能走索引的
   写法放好）。所以：**归一未落地 ⇒ 项目成员列表接口不可对外开放**。这一条以前只记在
   `open_verification` 里，第十轮 review 要求把它提到门禁级别，因为它不是"待核实"，
   是一个已知为假的前置条件。
2. **归一落地后必须打开 `OCTO_PROJECT_RECONCILE_ENABLED`。**
   五个受控扫描（I1 违约 / 清理泄漏 / 孤儿项目 / I4 缺群 / I4 缺员）默认关闭。前三条是
   因为在漂移下会报 1267，后两条是因为在漂移下的实测计划是 `group` 全表扫 + 临时表，
   而临时表会把它们自己的 ORDER BY / LIMIT 分页一起废掉。两类原因不同，出口相同：
   归一之前开着没有意义，归一之后**不开就等于没有监控**——而五个 gauge 停在 0 读起来
   与"零违约"完全一样。启动日志会为此打一条 Warn，那条 Warn 就是这条门禁的提醒。
