...
- `globalRateLimitExcludePaths()` 含 `/v1/bot/heartbeat` **与 `/v1/bot/register`**；
  `main_test.go` 同步断言两条都在（`TestGlobalRateLimitExcludesBothSelfHealChannels`），
  并另有守卫钉住"每条被豁免的 `/v1/bot/*` 都必须有自己的底线"
  （`TestExcludedBotPathsEachHaveTheirOwnFloor`）。
...
| 层 | 位置 | 职责 | 可关? |
|---|---|---|---|
| per-IP strict (`bot_heartbeat`, 500 rps / burst 1500) | **`authBot` 之前** | 未鉴权洪水底线,exclude 的对价 | **否** |
| `authBot` + `botActorUID` | 中 | 鉴权 + 落 bot 身份 | 否 |
| per-bot 桶 | `authBot` 之后 | 防单个已鉴权 bot 滥用 | 是(默认关) |

第一层**刻意不接 `enabled` 开关**:per-bot 桶默认关闭(灰度需要),若这层也可关,
exclude 之后就存在一个完全无防护的未鉴权端点。定值理由见下一节。

`register` 无此问题:它的 IP 桶已在最前,且该端点没有 `authBot`(handler 自己解析 token)。
主组亦无此问题:它**未**被 exclude,未鉴权洪水仍由全局 1500 rps 兜底。

### 两道 IP 底线的定值:按底线定,不按实测流量的小倍数定

第一版按单 IP 实测峰值各留两倍余量,得出 `register 10/50`、`heartbeat 100/300`。
**那是配额的定值方式,用错了对象**——这两道桶的职责只是"别让未鉴权流量无限打进来",
与"给正常业务分配额"对余量的要求差一个量级。两处都因此过紧:

- **heartbeat 余量只有 2.2 倍,且方向反了。** 反推车队规模:2903 个活跃 bot、
  心跳 key TTL 60s ⇒ 至少每 60s 一次、实际约 30s ⇒ **全车队心跳约 97 rps**。
  实测单 IP 45 rps 说明约半个车队在一个出网 IP 后面。车队翻倍或 NAT 收敛成单出口
  就顶满 100。在一个"被 429 就是事故本身"的端点上留 2.2 倍余量,与本任务目标矛盾。
- **register 是 150 倍收紧,而且落在自愈路径上。** 定这个值的时候它还在全局桶里,原上限
  就是全局的 1500 rps。压到 10 rps 后,车队集体重连时(唯一真正要紧的时刻)
  burst 只放过前 50 个、其余按 10/s 排队约 140 秒,而客户端已知把 429 与 400 混在
  一套重试策略里。这与本 brief 自己的结论"凡是用于恢复的端点都必须在保命集合里"
  直接冲突——保命集合里放一个按平峰量裁的上限,等于没放。

  > **后续修正(第三轮 review)**:register 后来也被移出了全局桶——正是因为留在桶里会
  > 让邻居 bot 打满配额时连坐它,#696 的死锁经 register 这条腿依然成立。上面这段
  > "还在全局桶里"是**定值当时**的前提,不是最终形态。定值结论不受影响:移出前
  > register 同时受全局 1500 与本底线 100 约束,紧约束本来就是 100。但移出后 100 是
  > 这条路径**唯一**的上限,没有外层兜底。
...
> **一般性教训**:同一个令牌桶,当"配额"用和当"底线"用,定值方法不同。
> 配额贴着实测走(要精确,靠影子数据收敛);底线要留数量级余量(它只需把无界变成有界)。
> 把底线按实测的两倍来定,会在正常流量的自然波动或规模增长下变成可用性故障——
> 而它拦下的那类滥用本来就不在实测量附近。
