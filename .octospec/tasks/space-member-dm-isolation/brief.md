<!-- NOT IMPLEMENTED IN PR #795. This is the hand-off spec for the DM half. -->

# Space member removal — DM isolation (follow-up)

> **Status: specification only. No code in this repository implements it yet.**
>
> This task was split out of `space-member-removal-cleanup` (PR #795) after eleven
> review rounds established two things:
>
> 1. **The DM half is inert in every current deployment.** WuKongIM's
>    `options.WhitelistOffOfPerson` defaults to `true`, and while it is true the send
>    path never consults the Person-channel whitelist. Measured end to end against the
>    CI-pinned broker; no deployment sets it. So `dm_cutoff` / `dm_restore` change no
>    message-delivery behaviour today.
> 2. **It does not converge by point fixes.** A new escape was found in six
>    consecutive rounds. The recurring root causes are structural, not local: peer
>    **scope** derived from one user's mutable conversation view, the **freshness** of
>    the authorization read, and the **channel form** the whitelist rule is applied to.
>
> Everything below is preserved from the original brief because it is measured rather
> than assumed — the broker probes, the seven-call-site completeness audit, and the
> escape analyses cost real work to produce. What ships here must additionally settle
> the open decisions listed under "Blocking decisions" and address the two P1s from
> review round 11 (recorded in Deviations).
>
> The implementation reviewed to round 11 is preserved on the branch
> `backup/dm-half-9932ab6` and in PR #795's history up to commit `9932ab6`.

## Blocking decisions this task must settle first

1. **`whitelistOffOfPerson`** — whether it is set to `false`, and by whom. Enabling it
   is not a pure config flip: nothing provisions whitelist entries for non-friend
   co-members on Space join, so flipping it today blocks DMs between co-members who
   are not friends. Join-time provisioning must land with it.
2. **Which channel form the whitelist rule is authoritative for.** The derivation is
   channel-form-agnostic; bot onboarding writes prefixed-only when a common Space
   exists. The two disagree, and the overwrite silently resolved it in the widening
   direction. Decide deliberately.
3. **The bilateral / authoritative pair index and the membership epoch.** Both
   remaining escape classes need them; neither is reachable by another point fix.

## Known P1s carried over from review round 11

- **Space-scoped restore write is guarded on only one of the two channels it writes.**
  `addSpaceScopedWhitelist(ctx, spaceID, peer, uid)` writes `s{spaceID}_{peer}` while
  the only membership check covers `uid`. `peer` is never checked against `spaceID`,
  and cannot be assumed a member (`MembersEverInSpace` is deliberately status-agnostic;
  `grantInbound` is true on friendship or on any *other* shared Space). Result: a
  Space-scoped inbound authorization for a non-member, written after that peer's own
  cleanup completed, with nothing pending to undo it.
- **Two "share an active Space?" predicates disagreed on the self case.**
  `HaveCommonSpace` returns true for `uid==uid`, the DM half's `SharesActiveSpace`
  returned false. Both were live isolation predicates 50 lines apart with no
  cross-reference. `SharesActiveSpace` is removed from this repo by the split; if it
  comes back, it must delegate or document the difference.
