---
type: Task
title: "Task: assistant-eager-ai-group"
description: Automatically register newly created assistant Bots as AI Team agents and eagerly provision their private two-member group.
tags: [space, isolation, auth, acl, thread, wire-contract, error-response, i18n, test]
timestamp: 2026-09-08T00:00:00+08:00
# --- octospec extension fields ---
slug: assistant-eager-ai-group
upstream: chat-request
source: self
---

# Task: assistant-eager-ai-group
...
Make “create assistant” converge to the complete Octo state expected by the client:
after the client creates the assistant's User Bot, it registers that Bot through
...
- The client currently selects an Octo Space, exchanges its XRUI token for a `uk_`
  User API Key, and calls `POST /v1/user/bots`.
- The client already maintains a separate Octo login Session Token through
...
- `test`, `testing`: update lifecycle expectations and add focused client request
...
- Idempotency and recovery: repeated Agent registration must reuse the same
  ...
  group.
- Cross-repository integration: octo-server and the client must agree on Bot ID,
  selected Space ID, and Octo Session Token usage.

## Out of scope

- Superseded by the later `ai-team-all-bots-group` brief: `POST /v1/user/bots`
  now uses the same idempotent Agent activation hook as the other production
  User Bot creation paths.
...
- The local assistant auto-connect flow calls the Agent endpoint after Bot
  creation with `token: <Octo Session Token>` and `X-Space-ID: <selected space>`.
  The Bot ID is URL-encoded and no token is logged.
- If the client cannot obtain Octo Session credentials or Agent registration fails,
...
- Focused Go tests, client Vitest coverage, changed-file ESLint, and Electron
