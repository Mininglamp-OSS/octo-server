...
- **D5 `fileMaxSizeKBHardCap = 524288`（512MB）**。更大的传输应走分片 / 网盘，
  不走 IM 附件通道。
  **D5 修订（2026-08-26）：天花板改由 env `OCTO_FILE_MAX_SIZE_KB_HARD_CAP` 决定，
  512MB 降级为未配置时的默认值。** 原设计把它写死成常量，是把 sticker 那组硬上限
  的模式套错了地方：「贴纸该多大」是产品语义边界，任何部署下都一样，写死正确；
  「单文件能传多大」是**部署容量决策**，取决于 ingress 的 body 限制、临时文件磁盘、
  带宽与对象存储后端，每个部署都不同，编译进二进制等于让代码替部署环境做一个
  它不知道答案的决定。而且本任务的立论就是「改上传策略不该发版」，天花板却要发版
  才能改，自相矛盾。
  仍然**不放进管理台**：天花板若也能从管理台改，超管可以先抬天花板、再抬实际值，
  这道闸就是摆设；env 要重启 Pod，这个成本本身是防线。职责因此分三层 ——
  开发定代码默认值、运维定天花板（configmap + 重启）、超管调实际值（管理台，≤60s）。
  非法 env 值（非数字 / ≤0 / 超过防溢出上界 100GB）回落代码默认值。
  新增 env 一律用 `OCTO_` 前缀；既有 `DM_FILE_EXTRA_*` 保持原名，改名会断现网配置。
...
## Review 修复记录（第四轮，2026-08-27）

三位 reviewer 在 head `597cb2c6` 上独立指向同一个 P1，并且都确认它是**本轮
commit（天花板 env 化）新引入的**，不是被本 PR 继承的既有问题：

- **天花板可低于贴纸上限，绕过了 D6 的组合守卫。** 天花板还是常量 524288 时，
  贴纸上限最高 5120，`clamp(x, 524288)` 永远落不到它之下 —— 「全局上限 < 贴纸
  上限」这个 D6 明确命名的致命组合在读侧**不可达**，所以 D6 只需要在写侧拦住
  超管的显式写入就够了。`OCTO_FILE_MAX_SIZE_KB_HARD_CAP` 让天花板可以配到贴纸
  上限之下，这条路径就打开了，而且**一次写入都不需要**：空表 + `=512` 开机即
  违规，写侧 merge-then-validate 按定义看不见一次没有发生过的写入。
  后果是三个面互相矛盾：闸门看得见天花板（贴纸被前置的全局门以「文件过大」
  拒掉），appconfig 与管理台 `effective_value` 都看不见（照旧广播 1024）。

**D6 修订：组合约束 = 读侧收敛 + 写侧拒绝，两层各管一件事。**

- **读侧收敛（新增）**：`StickerUploadMaxSizeKB()` 增加第二道上界 =
  `FileMaxSizeKB()`。这不是新政策，而是把既有事实写进代码 —— 上传校验里全局
  大小门本来就在贴纸门之前，真正生效的贴纸上限一直是 `min(两者)`；此前只有
  闸门知道这件事。收敛之后 appconfig、`effective_value`、贴纸门三面同源，
  不存在「写着但不生效」的数字，也不存在任何致命组合。收敛时按
  (贴纸值, 全局值) 去重打一条 Warn，运维能看见部署天花板正在压产品配置。
- **写侧拒绝（保留，修正比较口径）**：`ApplyFileStickerSizeOverlay` 叠加后
  按各自 getter 的上界钳位再比较。此前 `cur` 来自钳位过的 getter、`incoming`
  却是原样值，守卫校验的是一对运行时永远不会执行的组合：天花板是常量时这只会
  过度拒绝（fail-closed），可配之后会**漏拒** —— 天花板 512、贴纸默认 1024，
  超管写 `file.max_size_kb=4096` 得到 `200 {"applied":true}`，运行时钳位成
  `{512,1024}`。贴纸侧只钳到贴纸自身硬上限、**不钳全局上限**，否则两侧恒相等，
  守卫成为死代码。
  读侧收敛之后没有任何组合是致命的，写侧拒绝的意义因此收窄为**「意图不被静默
  改写」**：它是运营唯一能得知自己配的贴纸上限无法被兑现的地方。
  代价是低天花板部署下单独写 `file.max_size_kb` 会被拒，须与
  `sticker.upload_max_size_kb` 同批提交 —— 这正是该错误文案「请同时提交这两项」
  已经在说的做法，由 `TestManagerBatchWriteWithLoweredStickerIsAccepted` 钉死。
- **决定并钉死**：超过贴纸硬上限的写入是**钳位接受**，不是拒绝（钳到 5120 后
  若不与全局上限冲突即放行），见
  `TestApplyFileStickerSizeOverlay_OverCapStickerIsClampedAcceptance`。

**同族的两项（同样由本轮 commit 引入或放大）：**

- `clampIntUpper` 的 `v<=0` 分支直接 `return fallback`，从不过上界。天花板恒高于
  代码默认值时无害；可配到 100MB 以下之后，一个直改库的 ≤0 值会拿到 102400，
  高出部署自己声明的天花板 200 倍。改为 `min(fallback, hardCap)`。
- 天花板低于 100MB 且**表里一行都没有**时，每个 pod 都会打一条
  `configured=102400` —— 在诬告一次没有发生过的变更。新增 `getIntOK` 区分
  「配置过」与「用的是代码默认值」，未配置那一格不再走钳位器的告警分支。

**未采纳（留作独立任务，理由同第三轮）**：`policyInputs` 跨三次快照读、
`extra_allowed_extensions` 未过滤运维黑名单、解析出 0 项的非空 CSV 返回
`applied:true`、STS 凭证路径不受大小上限约束、bot/robot 缺 `MaxBytesReader`。
这五项**都早于本轮 commit**，与这次的 P1 无因果关系。本 PR 已经两次因为「顺手
多做」引入破坏性变更，边界因此划在「本轮 commit 引入的，就地修完；早于它的，
不在这里动」。
