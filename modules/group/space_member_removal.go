...
func (g *Group) handOverGroupCreator(groupNo, leaverUID, leaverName, spaceID string, suppressNotice bool) error {
	...
	//
	// 为什么串行下与执行顺序无关：一批工单是在**同一个事务**里原子入队的
	// （enqueueMemberRemovalCleanupBatchTx），所以任何 worker 开始跑之前，同批所有
	// 兄弟工单的 pending 行都已存在。因此不管工单以什么次序跑，「successor 不在队列里」
	// 在一次批量里只会对一个环成立。
	//
	// ⚠️ 多副本下**不是**绝对的。本检查跑在 handOverGroupCreator 提交之后，此时已不再
	// 持有继任者的行锁：若 worker A 在提交后、检查前停顿足够久（约 100ms 量级），
	// worker B 可以把 S2 的整条工单跑完并置为 done，A 随后读到 done → 不抑制 →
	...
	//
	// 没有在本次改动里做，理由是可达性：两个 goroutine 真并发跑同批兄弟工单 30 轮，
	// 多余通告 0 次——坏结果要求 A 在一条 COMMIT 和一次带索引的 COUNT 之间停顿到
	// 足够 B 跑完一整条工单（交接事务 + RemoveGroupMembers + 多次 IM 往返）。
	// 而确定性红测试需要在生产代码里加测试钩子。代价与收益不匹配，记为 follow-up。
	//
