...
func (d *DB) queryGroupCount() (int64, error) {
	var count int64
	_, err := d.session.Select("count(*)").From("`group`").Where("purpose='' OR purpose=?", aiteampkg.CustomTeamPurpose).Load(&count)
	return count, err
}
...
func (d *DB) queryCreatedCountWithDate(date string) (int64, error) {
	var count int64
	_, err := d.session.Select("count(*)").From("`group`").Where("date_format(created_at,'%Y-%m-%d')=? AND (purpose='' OR purpose=?)", date, aiteampkg.CustomTeamPurpose).Load(&count)
	return count, err
}
...
func (d *DB) querySameDayCreateCountWitUID(uid string, day string) (int, error) {
	var count int
	err := d.session.SelectBySql("SELECT COUNT(*) AS count FROM `group` WHERE creator=? AND DATE(created_at)=? AND (purpose='' OR purpose=?)", uid, day, aiteampkg.CustomTeamPurpose).LoadOne(&count)
	return count, err
}
