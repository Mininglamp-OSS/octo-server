...
//
// A Project owns no group. Every Project-linked group is an ordinary native
// group whose members, creator and IM subscriptions are independent of the
// Project roster: the 2026-09-24 cutover removed the Project-driven member
// projection, so the only group-side reaction left to a Project event is the
// disband fallback below (docs/specs/2026-09-10-project-prd-alignment-design.md
// 「Project 关联群的统一语义」).
...
// revertOneGroupToSpace clears one group's Project attribution and records the
// metric when the row actually changed. Idempotent through the update predicate.
