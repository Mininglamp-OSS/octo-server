...
// TestChannelGet_Person_DMForbidden_SurvivesMinimalStrip 钉住 dm_forbidden 的**下发链路**，
// 而不只是推导逻辑。
//
// 这个信号只经过两个环节就到客户端，而在这条用例之前，删掉任意一个整包测试都还是绿的：
//   - modules/user/1module.go 的 BussDataSource.ChannelGet 里那一次
//     annotateDMSendability 调用（生产里唯一的调用点，其余引用都是测试直接调函数）；
//   - modules/channel/service/profile_visibility.go 的 callerOwnedExtraKeys 中
//     dm_forbidden / dm_forbidden_reason 两个键——最小集是白名单，不在里面就被剥掉。
//
// 两个环节都断在「无关系」这条路径上，而无关系恰恰是这个标记唯一会被置位的场景：
// 推导正确、单测全绿，客户端却永远收不到，输入框照常可写、消息被 WuKongIM 静默拒收。
// 与 TestRejoinRestoreStepRunsOnJoinPath 同一类：走真实路由，不调函数。
//
// 正反两面都断言，避免「永远返回 forbidden」也能让用例通过。
func TestChannelGet_Person_DMForbidden_SurvivesMinimalStrip(t *testing.T) {
	s, ctx := newChannelAuthzServer(t)
	defer testutil.CleanAllTables(ctx)

	// ① 无关系（非好友、无共同 Space）→ 走最小集，且标记必须活着穿过最小集。
	assert.NoError(t, user.NewService(ctx).AddUser(&user.AddUserReq{
		UID: "t_dmf_stranger", Name: "DMFStranger", ShortNo: "SNDMF1",
	}))
	w := getChannel(s, "t_dmf_stranger", common.ChannelTypePerson.Uint8())
	assert.Equal(t, http.StatusOK, w.Code, "body=%s", w.Body.String())
	assert.Contains(t, w.Body.String(), "\"dm_forbidden\":1",
		"无关系必须下发 dm_forbidden——删掉 annotateDMSendability 调用点或把该键移出 "+
			"callerOwnedExtraKeys，这里就该红, body=%s", w.Body.String())
	assert.Contains(t, w.Body.String(), "\"dm_forbidden_reason\"",
		"原因字段与标记同进同出, body=%s", w.Body.String())

	// ② 同 Space 共处 → 可发送，标记不得出现。
	// 没有这一半，一个「无条件置位」的实现也能让①通过。
	assert.NoError(t, user.NewService(ctx).AddUser(&user.AddUserReq{
		UID: "t_dmf_mate", Name: "DMFMate", ShortNo: "SNDMF2",
	}))
	seedSpace(t, ctx, "sp_dmf", 1) // 必须是活跃 Space：SharesActiveSpace 要求 space.status=1
	seedSpaceMember(t, ctx, "sp_dmf", testutil.UID)
	seedSpaceMember(t, ctx, "sp_dmf", "t_dmf_mate")
	w2 := getChannel(s, "t_dmf_mate", common.ChannelTypePerson.Uint8())
	assert.Equal(t, http.StatusOK, w2.Code, "body=%s", w2.Body.String())
	assert.NotContains(t, w2.Body.String(), "dm_forbidden",
		"共处同一活跃 Space 时可以发消息，不得置灰, body=%s", w2.Body.String())
}
