...
//
// Pinning is a personal preference, not a membership fact. Keeping it in its own
// table means a pin does not touch octo_project_member or member_epoch, which fleet
// and drive read to decide whether membership changed and which
// TestMemberEpochOnlyEverIncrements pins to +1 steps. A pin is not a membership
// change and must never move it. The migration header argues both at length;
// repeated here because the next person to add a preference will be looking at this
// file, not at the SQL.
...
// Project seat is active and not closing.
const sqlCountPinnedInSpace = "SELECT COUNT(*) FROM octo_project_user_setting s " +
	...
	"  ON pm.project_id = p.project_id AND pm.space_id = p.space_id AND pm.uid = s.uid " +
	"WHERE p.space_id = ? AND p.status = ? AND pm.status = ? AND pm.removing = 0 " +
	"  AND s.uid = ? AND s.pinned = 1"
...
//
// octo_project_user_setting deliberately carries no space_id. The quota joins
// through octo_project to resolve each pinned Project's Space, so the setting row
// does not duplicate the Project's space_id fact.
//
// Not "every pinned row". The quota predicate mirrors the Project list predicate:
// p.status is normal, the Space matches, and the caller has an active Project seat
// whose removal is not in progress. A row for a Project whose seat is closing or
// removed is no longer actionable through the membership-only Project routes and
// must not consume a slot.
//
// An active member's pin remains counted because that same membership row keeps the
// Project in the caller's list. The count is only a quota check; it does not grant
// access or widen the settings endpoint beyond the Project membership gate.
//
...
func (d *DB) countPinnedInSpaceTx(tx *dbr.Tx, spaceID, uid string) (int, error) {
	...
	err := runner.SelectBySql(sqlCountPinnedInSpace,
		spaceID, StatusNormal, MemberStatusActive, uid,
	).LoadOne(&n)
	if err != nil {
