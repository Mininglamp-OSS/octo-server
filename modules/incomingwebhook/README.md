...
- **一切仍锚父群**：鉴权/成员/群解散级联、`@` 成员闸与 `@所有 AI` 展开都仍以父群
  `group_no` 为准（子区成员完全派生自父群）。唯一变化是消息投递的目标频道。
  **配额例外**：数量配额按投递作用域 `(group_no, thread_short_id)` 计——群本体与每个
  子区各有独立的 `max_per_group` 名额（详见下方「配额」）；串行化锁仍锚父群行。
