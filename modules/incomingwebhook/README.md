...
- `username` / `avatar_url`：可选，覆盖该条消息的展示发送者名/头像（不改 webhook 本身
  ...
  两个字段被静默忽略（推送仍成功），展示固定为存量 Name/Avatar——即 create/update
  端点已配置好的值（头像通常是成员路径的确定性默认头像，但管理员可为任意 webhook
  单独设置自定义头像，见上文「头像」小节，届时该自定义值同样固定生效）；否则这些
  端点已配置好的身份会被 push 路径整体绕过。判权结果随创建者现任角色，变更生效延迟
  ≤ 一个缓存 TTL（默认 3s）。
