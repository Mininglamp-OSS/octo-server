...
func TestMain(m *testing.M) {
	...
	depDDLs := []string{
		...
		"CREATE TABLE robot (id BIGINT AUTO_INCREMENT PRIMARY KEY, robot_id VARCHAR(40) NOT NULL DEFAULT '', token VARCHAR(200) DEFAULT '', status SMALLINT NOT NULL DEFAULT 1, creator_uid VARCHAR(40) NOT NULL DEFAULT '', description VARCHAR(500) NOT NULL DEFAULT '', auto_approve SMALLINT NOT NULL DEFAULT 0, kind VARCHAR(16) NOT NULL DEFAULT 'user', management_scope VARCHAR(16) NOT NULL DEFAULT '', management_space_id VARCHAR(40) NOT NULL DEFAULT '', created_by VARCHAR(40) NOT NULL DEFAULT '', publication_state VARCHAR(16) NOT NULL DEFAULT 'draft', lifecycle_pending TINYINT NOT NULL DEFAULT 0, agent_hosting VARCHAR(64) NOT NULL DEFAULT '', agent_reported_hosting_at TIMESTAMP NULL DEFAULT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, UNIQUE KEY idx_robot_id(robot_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci",
