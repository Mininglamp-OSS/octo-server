...
// directoryFromJoin 返回 count 与分页查询共用的 FROM/JOIN 片段。
// 所有 LEFT JOIN 都命中唯一键（user.uid、user_verification 的主键 user_id、
// robot.robot_id 唯一索引），因此不会扇出，一个 uid 恒对应一行。
//
// type=human 时省略 robot 表：`u.robot = 0` 已经排除了全部 bot 账号——无论其
// 启用与否——所以 robot join 对这个切片没有任何判定作用。实测每行都要做一次
// eq_ref 单行查找，6000 成员的 Space 上这一个 join 就是约 12ms 的纯浪费。
func directoryFromJoin(listType string) string {
	join := `
...
// directorySelectList 返回投影列表。active_bot 的取法随切片而变：type=human 不
// join robot 表，SQL 里不能引用 r，该列恒为 0——这与语义一致，人类行本就不是 bot。
func directorySelectList(listType string) string {
	activeBot := `CASE WHEN r.robot_id IS NOT NULL THEN 1 ELSE 0 END`
	if listType == directoryTypeHuman {
		activeBot = `0`
	}
	return `
...
func (d *DB) queryDirectory(spaceID, listType string, page, limit uint64) ([]*directoryRowModel, error) {
	...
	_, err := d.session.SelectBySql(
		directorySelectList(listType)+
			directoryFromJoin(listType)+` WHERE `+where+directoryOrderBy+` LIMIT ? OFFSET ?`,
		args...).Load(&models)
	return models, err
}
...
func (d *DB) countDirectory(spaceID, listType string) (int64, error) {
	...
	var count int64
	_, err := d.session.SelectBySql(
		`SELECT COUNT(*)`+directoryFromJoin(listType)+` WHERE `+where, args...).Load(&count)
	return count, err
}
