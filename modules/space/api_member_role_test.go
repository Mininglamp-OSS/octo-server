...
func TestTransferOwnerAdminTargetRemoved(t *testing.T) {
	...
	_, err = f.db.removeMemberLocked(spaceId, "m-target", 2, testutil.UID, MemberRemoveReasonKicked)
	assert.NoError(t, err)
...
func TestRemoveMemberLockedGuards(t *testing.T) {
	...
	_, err = f.db.removeMemberLocked(spaceId, testutil.UID, 2, testutil.UID, MemberRemoveReasonLeft)
	assert.ErrorIs(t, err, ErrCannotRemoveOwner)
	owner, _ := f.db.queryMember(spaceId, testutil.UID)
	assert.Equal(t, 2, owner.Role)

	// 操作者 admin(1) 移除 admin(1)：同级拒绝
	_, err = f.db.removeMemberLocked(spaceId, "m-target", 1, testutil.UID, MemberRemoveReasonKicked)
