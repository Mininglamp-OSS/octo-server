...
func TestStoreActivateCreatesFirstRevisionAndAuditAtomically(t *testing.T) {
	...
	expectActiveTargetCapacity(t, mock, 0)
...
func TestStoreActivateRejectsWhenActiveTargetCapacityIsFull(t *testing.T) {
	store, mock, closeDB := newMockStore(t)
	defer closeDB()
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT active_version, status, revision")).
		WithArgs("test.runtime-card").
		WillReturnError(sql.ErrNoRows)
	expectActiveTargetCapacity(t, mock, startupActiveTargetLimit)
	mock.ExpectRollback()

	_, err := store.Activate(context.Background(), activationRequestFixture())
	if !errors.Is(err, ErrActiveTargetCapacity) {
		t.Fatalf("Activate error = %v, want typed capacity conflict", err)
	}
	assertMockExpectations(t, mock)
}
...
func TestStoreBlockCurrentVersionWithoutFallbackDisablesAtomically(t *testing.T) {
	...
	expectActiveTargetCapacityGuard(t, mock)
	mock.ExpectQuery(regexp.QuoteMeta("SELECT c.source, a.template_id IS NOT NULL, a.blocked_at IS NOT NULL")).
		WithArgs("test.runtime-card", "1.1.0").
		WillReturnRows(dynamicStateTargetRows(strings.Repeat("a", 64)))
...
func TestStoreRollbackRejectsDisabledToActiveWhenCapacityIsFull(t *testing.T) {
	store, mock, closeDB := newMockStore(t)
	defer closeDB()
	...
	mock.ExpectQuery(regexp.QuoteMeta("SELECT active_version, status, revision")).
		WithArgs("test.runtime-card").
		WillReturnRows(sqlmock.NewRows([]string{"active_version", "status", "revision"}).
			AddRow(nil, activationStatusDisabled, 4))
	expectActiveTargetCapacity(t, mock, startupActiveTargetLimit)
	mock.ExpectRollback()
	...
	_, err := store.Rollback(context.Background(), RollbackRequest{
		...
		ExpectedRevision: 4,
		ActorUID:         "admin-1",
		Reason:           "rollback disabled target",
		ChangeTicket:     "INC-3",
		targetReceipt:    dynamicTargetReceiptFixture("test.runtime-card", "1.0.0", strings.Repeat("a", 64)),
	})
	if !errors.Is(err, ErrActiveTargetCapacity) {
		t.Fatalf("Rollback error = %v, want typed capacity conflict", err)
	}
	assertMockExpectations(t, mock)
}
...
func TestActivationMigrationDefinesCASStateAndAuditColumns(t *testing.T) {
	...
	for _, required := range []string{
		...
		"INSERT INTO card_template_catalog_capacity_guard",
		"active_version",
		"status",
...
func expectActiveTargetCapacity(t *testing.T, mock sqlmock.Sqlmock, active int) {
	t.Helper()
	expectActiveTargetCapacityGuard(t, mock)
	mock.ExpectQuery(regexp.QuoteMeta("SELECT COUNT(*) FROM card_template_activation")).
		WillReturnRows(sqlmock.NewRows([]string{"active_count"}).AddRow(active))
}

func expectActiveTargetCapacityGuard(t *testing.T, mock sqlmock.Sqlmock) {
	t.Helper()
	mock.ExpectQuery(regexp.QuoteMeta("SELECT guard_key FROM card_template_catalog_capacity_guard")).
		WillReturnRows(sqlmock.NewRows([]string{"guard_key"}).AddRow(1))
}
