...
### Incoming Webhook 投递到子区

外部服务可创建绑定到子区的入站 webhook，推送直接投递进子区频道。管理端点
`/v1/groups/{group_no}/threads/{short_id}/incoming-webhooks`（含 bot 面），鉴权/成员/配额
仍锚父群，推送 URL/body 与群 webhook 一致。契约详见 `modules/incomingwebhook/README.md`。
