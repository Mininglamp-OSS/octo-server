...
func TestMigratedUserFilesNoLegacyResponseError(t *testing.T) {
	files := []string{
		...
		"scanlogin_poll.go", "api_manager_2fa.go",
	}
	for _, f := range files {
...
func TestRespondUserHelpers(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			wantSemStatus:   http.StatusBadRequest,
			wantTransStatus: http.StatusBadRequest,
			wantContains:    "密码长度至少为 8 位",
		},
		{
			name:            "ErrUserPasswordTooWeak surfaces zh-CN copy",
			probe:           func(c *wkhttp.Context) { respondUserError(c, errcode.ErrUserPasswordTooWeak) },
			wantCodeID:      "err.server.user.password_too_weak",
			wantSemStatus:   http.StatusBadRequest,
			wantTransStatus: http.StatusBadRequest,
			wantContains:    "至少 2 类",
		},
		{
			name:            "ErrUserPasswordTooLong surfaces zh-CN copy",
			probe:           func(c *wkhttp.Context) { respondUserError(c, errcode.ErrUserPasswordTooLong) },
			wantCodeID:      "err.server.user.password_too_long",
			wantSemStatus:   http.StatusBadRequest,
			wantTransStatus: http.StatusBadRequest,
			wantContains:    "不能超过 72 字节",
		},
		{
