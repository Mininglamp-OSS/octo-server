...
// TestPurgeRemovedMigrationIDs covers the deletion shim that handles
// gorp_migrations rows whose source SQL files were removed by a module
// deletion (issue #139: modules/backup). The contract: a fresh install
// (table absent) is a no-op so module.Setup can create the table after;
// rows present in the removed list get deleted in a single transaction;
// rows absent from the table are skipped (idempotent restart).
func TestPurgeRemovedMigrationIDs(t *testing.T) {
	if len(removedMigrationIDs) == 0 {
		t.Skip("no removed migration IDs configured — skip")
	}
	// Pick the first entry as a deterministic fixture; sort the full list
	// the same way the implementation does so expectation order matches.
	sortedRemoved := append([]string(nil), removedMigrationIDs...)
	sort.Strings(sortedRemoved)
	firstID := sortedRemoved[0]
	...
	t.Run("table absent — fresh install no-op", func(t *testing.T) {
		...
		if err := PurgeRemovedMigrationIDs(context.Background(), db); err != nil {
			t.Fatalf("expected nil for absent table, got %v", err)
		}
		mustExpectationsMet(t, mock)
	})

	t.Run("no removed rows present — idempotent no-op", func(t *testing.T) {
		db, mock := openMock(t)
		defer db.Close()
		...
		mock.ExpectQuery(regexp.QuoteMeta(
			"SELECT TABLE_NAME FROM information_schema.TABLES")).
			WillReturnRows(sqlmock.NewRows([]string{"TABLE_NAME"}).AddRow("gorp_migrations"))
		// Existing rows do not include any removed IDs.
		mock.ExpectQuery(regexp.QuoteMeta("SELECT id FROM gorp_migrations")).
			WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow("20260513000001_conversation_ext_legacy01.sql"))

		if err := PurgeRemovedMigrationIDs(context.Background(), db); err != nil {
			t.Fatalf("expected nil when no purge targets present, got %v", err)
		}
		mustExpectationsMet(t, mock)
	})

	t.Run("removed rows present — delete all in one tx", func(t *testing.T) {
		db, mock := openMock(t)
		defer db.Close()

		rows := sqlmock.NewRows([]string{"id"})
		for _, id := range sortedRemoved {
			rows.AddRow(id)
		}
		// Add an unrelated row to assert it is left alone.
		rows.AddRow("20260513000001_conversation_ext_legacy01.sql")
		...
		stmt := mock.ExpectPrepare(regexp.QuoteMeta(
			"DELETE FROM gorp_migrations WHERE id = ?"))
		// Deterministic order — sort.Strings on toDelete inside the impl.
		for _, id := range sortedRemoved {
			stmt.ExpectExec().WithArgs(id).
				WillReturnResult(sqlmock.NewResult(0, 1))
		}
		mock.ExpectCommit()
	...
	t.Run("partial — only some removed rows present", func(t *testing.T) {
		db, mock := openMock(t)
		defer db.Close()
		...
		stmt := mock.ExpectPrepare(regexp.QuoteMeta(
			"DELETE FROM gorp_migrations WHERE id = ?"))
		stmt.ExpectExec().WithArgs(firstID).
			WillReturnResult(sqlmock.NewResult(0, 1))
		mock.ExpectCommit()
