...
// PushDisplayText 为离线推送/存储后展示路径产出卡片文案：优先信任 payload 里
// server 已生成的权威 plain（Decision 8 保证经校验路径永不为空）；缺失/为空时
// 现场从 card 重算兜底；再不行回退 [卡片]。
//
// ⚠️ Decision 2 residual-risk 规则（sender 非 bot/webhook 身份的 type-17 不得
// 信任 plain,须直接用 [卡片] 占位）需要 sender 身份判定,由调用方在拿到
// from_uid 身份后决定是否改用 DisplayText() —— 本函数只负责「可信路径」的取文案。
