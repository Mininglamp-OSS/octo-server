...
//   - 序列化总量 ≤ 16KiB。
//
// envelopeRaw 是「生效卡片」信封字节（content_edit 优先，与 SubmitActionIDs
// 的取帧口径一致 —— 校验对象必须是用户实际看到的那一帧）。
...
// SubmitActionIDs 相同（body 内嵌套 Container/ColumnSet 均收集）。解析失败
// 返回空集 —— 查无声明即拒绝（fail-closed）。
